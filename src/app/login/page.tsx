...
import { Sparkles, Mail, Lock, UserPlus, LogIn, Ghost } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleAuth = (e: React.FormEvent) => {
    ...
    // Redirect is handled by onAuthStateChanged in the provider, 
    // but we can give a nudge or just wait.
    router.push('/');
  };
      ...
      <Card className="w-full max-w-md border-none shadow-2xl rounded-3xl overflow-hidden">
        ...
        <CardContent className="pt-8 space-y-6">
          <form onSubmit={handleAuth} className="space-y-4">
            ...
            <Button type="submit" className="w-full h-12 rounded-full bg-rose-gold hover:bg-rose-gold/90 text-white font-bold text-lg">
          ...
          >
            <Ghost className="mr-2 h-5 w-5" /> Anonymous Access
          </Button>
        </CardContent>
